...
export default function QuickBillPage() {
  ...
  return <FeaturePage active="/quick-bill" eyebrow="FAST ENTRY" title="Quick bill" description="Make a small bill directly for a company without opening the full purchase or sale register.">
    ...
    </div><div className={styles.billTotal}><span>Total bill amount</span><strong>{money.format(total)}</strong><small>{billType} · Remaining: {money.format(remaining)} will be tracked in Udhari / Outstanding</small></div><div className={styles.billActions}><button className={styles.saveButton}>Save quick bill</button><button className={styles.secondaryButton} type="button" onClick={() => setShowPreview((current) => !current)}>{showPreview ? "Hide bill" : "View bill"}</button><a className={styles.whatsappButton} href={whatsappLink} target="_blank" rel="noreferrer">Send on WhatsApp</a></div></section>
