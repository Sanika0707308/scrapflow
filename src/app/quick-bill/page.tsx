...
export default function QuickBillPage() {
  ...
  const downloadPdf = () => {
    ...
    right(money.format(total), itemEnd, 9);
    left(`${quantity} × ${money.format(Number(rate) || 0)}`, itemEnd + 6, 8);
    line(itemEnd + 11);
    left("Bill total", itemEnd + 19, 9); right(money.format(total), itemEnd + 19, 9);
    left("Paid now", itemEnd + 27, 9); right(money.format(Number(paymentReceived) || 0), itemEnd + 27, 9);
    left("BALANCE / UDHARI", itemEnd + 36, 9); right(money.format(remaining), itemEnd + 36, 10);
    line(itemEnd + 42);
